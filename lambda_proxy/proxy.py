"""lambda_proxy.proxy: translate request from AWS api-gateway
...
class RouteEntry(object):
    ...
    def __init__(self, view_function, view_name, path, methods, cors=False, token=False):
...
class API(object):
    ...
    def _validate_token(self, params):
        """Validate token
        This is a simple token verification
        """
    ...
    def __call__(self, event, context):
        ...
        if route_entry.token:
            ...
            if not self._validate_token(params):
                return self.response('ERROR', 'application/json', json.dumps({"message": "Forbidden"}))
